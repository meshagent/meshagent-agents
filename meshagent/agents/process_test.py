...
def test_llm_agent_process_adds_mcp_proxy_config_from_environment(monkeypatch) -> None:
    monkeypatch.setenv("MESHAGENT_API_KEY", "runtime-api-key")
    monkeypatch.setenv("MESHAGENT_API_URL", "ws://router.internal:8080")
    room = _DownloadRecordingRoom()
    process = _make_llm_agent_process(
        room=room,
        thread_id="/threads/test.thread",
